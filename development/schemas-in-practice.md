...
You write schemas in `foundation/schemas/`, reference them as `@/product`, keep records in `records/`, and deploy the built site anywhere. Standards like `@std/person` work too — add `@uniweb/schemas` as a foundation dependency. No account, no registration, no network.
